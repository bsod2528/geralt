...
class Developer(commands.Cog):
    ...
    async def add_to_blacklist(self, ctx: BaseContext, snowflake: discord.Object, reason: str, url: str):
        ...
        if self.bot.get_channel(snowflake.id):
            try:
                await self.bot.db.execute(query, snowflake.id, "channel", reason, discord.utils.utcnow(), url)
        ...
        if self.bot.get_user(snowflake.id):
            try:
                ...
                try:
                    ...
                    return
                await self.bot.db.execute(query, snowflake.id, "user", reason, discord.utils.utcnow(), url)
                return self.bot.blacklists.add(snowflake.id)
            except Exception as exception:
                return await ctx.send(f"```py\n{exception}\n```")
        ...
        if self.bot.get_guild(snowflake.id):
            try:
                await self.bot.db.execute(query, snowflake.id, "guild", reason, discord.utils.utcnow(), url)
                return self.bot.blacklists.add(snowflake.id)
            except Exception as exception:
                return await ctx.send(f"```py\n{exception}\n```")
        ...
        async def yes(ui: discord.ui.View, interaction: discord.Interaction, button: discord.ui.button):
            ...
            async with aiohttp.ClientSession() as session:
                death_webhook = discord.Webhook.partial(
                    id=CONFIG.get("NOTIF_ID"), token=CONFIG.get("NOTIF_TOKEN"), session=session)
    ...
    @commands.is_owner()
    @commands.cooldown(2, 5, commands.BucketType.user)
    async def dm(self, ctx: BaseContext, user: discord.User, *, message: str) -> Optional[discord.Message]:
    ...
    @commands.is_owner()
    @commands.cooldown(2, 5, commands.BucketType.user)
    async def reload(self, ctx: BaseContext, *, cog: str = None) -> Optional[discord.Message]:
        """Reloads the Extension mentioned."""
        if cog is None:
            try:
                for cogs in COGS_EXTENSIONS:
                    await self.bot.reload_extension(cogs)
                await ctx.reply(f"Reloaded `{len(cogs)}` files <:RavenPray:914410353155244073>", allowed_mentions=self.bot.mentions)
            except Exception as exception:
                async with ctx.channel.typing():
                    await asyncio.sleep(0.1)
    ...
    @commands.cooldown(2, 5, commands.BucketType.user)
    @commands.is_owner()
    async def guild_fetch(self, ctx: BaseContext, *, guild: discord.Guild) -> Optional[discord.Message]:
    ...
    @commands.cooldown(2, 5, commands.BucketType.user)
    async def blacklist_add(self, ctx: BaseContext, snowflake: discord.Object, *, reason: str = None) -> Optional[discord.Message]:
    ...
    @commands.cooldown(2, 5, commands.BucketType.user)
    async def blacklist_remove(self, ctx: BaseContext, snowflake: discord.Object) -> Optional[discord.Message]:
        """Remove objects from blacklist."""
        await self.remove_from_blacklist(ctx, snowflake)

    @blacklist.command(
        name="all",
        brief="Sends all blacklisted objects.")
    @commands.cooldown(2, 5, commands.BucketType.user)
    ...
    @commands.command(
        name="self-purge")
    @commands.is_owner()
    async def de(self, ctx: BaseContext, limit: int = None) -> Optional[discord.Message]:
