...
class Moderation(commands.Cog):
    ...
    async def kick(self, ctx: BaseContext, user: discord.Member, *, reason: str = "Not Provided") -> Optional[discord.Message]:
    ...
    async def ban(self, ctx: BaseContext, user: discord.Member, *, reason: str = "Not Provided") -> Optional[discord.Message]:
    ...
    @app_commands.describe(reason="Reason why you're banning them.")
    @app_commands.describe(user="User you want to ban from this server.")
    async def unban(self, ctx: BaseContext, user: discord.Member, *, reason: str = "Not Provided") -> Optional[discord.Message]:
        ...
        async def yes(ui: discord.ui.View, interaction: discord.Interaction, button: discord.ui.button):
            ...
            for view in ui.children:
                view.disabled = True
            await interaction.response.edit_message(content=f"\u2001", embed=unban_emb, view=ui)

        async def no(ui: discord.ui.View, interaction: discord.Interaction, button: discord.ui.button):
            for view in ui.children:
                view.disabled = True
            await interaction.response.edit_message(content=f"**{user.mention}** will not be unbanned.", allowed_mentions=self.bot.mentions, view=ui)

        Confirmation.response = await ctx.send(f"Are you sure you want to unban {user.mention}", view=Confirmation(ctx, yes, no), allowed_mentions=self.bot.mentions)
    ...
    async def mute(self, ctx: BaseContext, user: discord.Member, *, reason: str = "Not Provided") -> Optional[discord.Message]:
        """Mute toxic users"""
        self.check_hierarchy(ctx, user)
    ...
    async def unmute(self, ctx: BaseContext, user: discord.Member, *, reason: str = "Not Provided") -> Optional[discord.Message]:
        """Unmute users"""
        self.check_hierarchy(ctx, user)
        role = discord.utils.get(ctx.guild.roles, name="Muted")
    ...
    async def setnick(self, ctx: BaseContext, user: discord.Member, *, nick: str) -> Optional[discord.Message]:
    ...
    async def purge(self, ctx: BaseContext, *, limit: Optional[int]) -> Optional[discord.Message]:
        ...
        if ctx.interaction:
            await ctx.channel.purge(limit=limit, bulk=False, before=ctx.interaction.created_at)
    ...
    async def lock(self, ctx: BaseContext, channel: discord.TextChannel, snowflake: Union[discord.Member, discord.Role]) -> Optional[discord.Message]:
    ...
    async def channel_unlock(self, ctx: BaseContext, channel: discord.TextChannel, snowflake: Union[discord.Member, discord.Role]) -> Optional[discord.Message]:
