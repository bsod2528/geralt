...
class Moderation(commands.Cog):
    ...
    @staticmethod
    def check_hierarchy(ctx: BaseContext, user: discord.Member):
    ...
    async def kick(
        self, ctx: BaseContext, user: discord.Member, *, reason: str = "Not Provided"
        ...
        async def yes(
            ...
            await interaction.response.edit_message(
                content=f"\u2001", embed=kick_emb, view=ui
            )
        ...
        async def no(
            ...
            for view in ui.children:
                view.disabled = True
            await interaction.response.edit_message(
                content=f"**{user.mention}** will not be kicked.",
                allowed_mentions=self.bot.mentions,
                view=ui,
            )

        Confirmation.response = await ctx.send(
            f"Are you sure you want to kick {user.mention}",
            view=Confirmation(ctx, yes, no),
            allowed_mentions=self.bot.mentions,
        )

    @commands.hybrid_command(name="ban", brief="Bans User")
    @app_commands.checks.cooldown(5, 3)
    ...
    async def ban(
        ...
        async def yes(
            ...
            await ctx.guild.ban(user=user, reason=f"{user} - {reason} by {ctx.author}")
            ban_emb = BaseEmbed(
            ...
            for view in ui.children:
                view.disabled = True
            await interaction.response.edit_message(
                content=f"\u2001", embed=ban_emb, view=ui
            )
        ...
        async def no(
            ...
            for view in ui.children:
                view.disabled = True
            await interaction.response.edit_message(
                content=f"**{user.mention}** will not be banned.",
                allowed_mentions=self.bot.mentions,
                view=ui,
            )
    ...
    @commands.hybrid_command(name="unban", brief="Unbans User")
    @app_commands.checks.cooldown(5, 3)
    @commands.has_guild_permissions(ban_members=True)
    @commands.cooldown(5, 3, commands.BucketType.user)
    @app_commands.describe(reason="Reason why you're banning them.")
    ...
    async def unban(
        ...
        async def yes(
            ...
            await ctx.guild.unban(user=user, reason=f"{reason} by {ctx.author}")
            unban_emb = BaseEmbed(
                title="UnBan - Has Occured.",
                description=f">>> {user.mention} has been **unbanned** <a:Banned:941667204334764042> !\n<:ReplyContinued:930634770004725821>** - ID :** `{user.id}`\n<:Reply:930634822865547294>** - On :** {self.bot.timestamp(ctx.message.created_at, style = 'F')}",
                colour=self.bot.colour,
            )
            ...
            for view in ui.children:
                view.disabled = True
            await interaction.response.edit_message(
                content=f"\u2001", embed=unban_emb, view=ui
            )
        ...
        async def no(
            ...
            for view in ui.children:
                view.disabled = True
            await interaction.response.edit_message(
                content=f"**{user.mention}** will not be unbanned.",
                allowed_mentions=self.bot.mentions,
                view=ui,
            )

        Confirmation.response = await ctx.send(
            f"Are you sure you want to unban {user.mention}",
            view=Confirmation(ctx, yes, no),
            allowed_mentions=self.bot.mentions,
        )

    @commands.hybrid_command(name="mute", brief="Mutes User")
    @app_commands.checks.cooldown(5, 3)
    @commands.cooldown(5, 3, commands.BucketType.user)
    ...
    async def mute(
        ...
        if role in user.roles:
            await ctx.send(
                f"**{user}** already has the role and is currently muted <a:LifeSucks:932255208044650596>"
            )
        else:
            ...
            async def yes(
                ...
                if not role:
                    create_role = await ctx.guild.create_role(
                        name="Muted",
            ...
            async def no(
                ...
                for view in ui.children:
                    view.disabled = True
                await interaction.response.edit_message(
                    content=f"**{user.mention}** will not be muted.",
                    allowed_mentions=self.bot.mentions,
                    view=ui,
                )

        Confirmation.response = await ctx.send(
            f"Are you sure you want to **mute** {user.mention}\n<:Reply:930634822865547294> **- For :** {reason}",
            view=Confirmation(ctx, yes, no),
            allowed_mentions=self.bot.mentions,
        )

    @commands.hybrid_command(name="unmute", brief="Unmutes User")
    @app_commands.checks.cooldown(5, 3)
    @commands.cooldown(5, 3, commands.BucketType.user)
    @commands.has_guild_permissions(manage_roles=True)
    ...
    async def unmute(
        ...
        async def no(
            ...
            for view in ui.children:
                view.disabled = True
            await interaction.response.edit_message(
                content=f"**{user.mention}** will not be unmuted.",
                allowed_mentions=self.bot.mentions,
                view=ui,
            )

        Confirmation.response = await ctx.send(
            f"Are you sure you want to **unmute** {user.mention}\n<:Reply:930634822865547294> **- For :** {reason}",
            view=Confirmation(ctx, yes, no),
            allowed_mentions=self.bot.mentions,
        )

    @commands.hybrid_command(name="setnick", brief="Change Nick", aliases=["nick"])
    @app_commands.checks.cooldown(5, 3)
    ...
    async def setnick(
        self, ctx: BaseContext, user: discord.Member, *, nick: str
    ) -> Optional[discord.Message]:
        ...
        await ctx.reply(
            f"<:GeraltRightArrow:904740634982760459> {user.mention}'s nickname has been `changed` -\n>>> <:ReplyContinued:930634770004725821> - From : {previous_nickname}\n<:Reply:930634822865547294> - To : {new_nickname} \n**Event Occured On :** {self.bot.timestamp(discord.utils.utcnow(), style = 'F')} <a:IEat:940413722537644033>",
            allowed_mentions=self.bot.mentions,
        )

    @commands.hybrid_command(name="purge", brief="Purge messages", aliases=["cls"])
    @app_commands.checks.cooldown(5, 3)
    @commands.cooldown(5, 3, commands.BucketType.user)
    @commands.has_guild_permissions(manage_messages=True)
    @app_commands.describe(limit="Number of messages you want to delete.")
    async def purge(
        self, ctx: BaseContext, *, limit: Optional[int]
    ) -> Optional[discord.Message]:
        ...
        if ctx.interaction:
            await ctx.channel.purge(
                limit=limit, bulk=False, before=ctx.interaction.created_at
            )
    ...
    @commands.hybrid_group(
        name="channel", brief="Manage Channels", aliases=["ch"], with_app_command=True
    )
    @commands.bot_has_guild_permissions(manage_channels=True)
    async def channel(self, ctx: BaseContext) -> Optional[discord.Message]:
    ...
    @app_commands.describe(
        snowflake="The role or user you want to block that channel for."
    )
    async def lock(
        self,
        ctx: BaseContext,
        channel: discord.TextChannel,
        snowflake: Union[discord.Member, discord.Role],
    ) -> Optional[discord.Message]:
        ...
        async def yes(
            ...
            try:
                ...
                data = await self.bot.db.fetchval(
                    "SELECT object_id FROM channel_lock WHERE object_id = $1 AND guild_id = $2",
                    snowflake.id,
                    ctx.guild.id,
                )
            if data:
            ...
            await ui.response.edit(
                embed=locked_emb, view=ui, allowed_mentions=self.bot.mentions
            )
        ...
        async def no(
            ...
            try:
                ...
                pass
            await ui.response.edit(
                embed=wont_lock_emb, view=ui, allowed_mentions=self.bot.mentions
            )
    ...
    @app_commands.describe(
        snowflake="The role or user you want to unblock that channel for."
    )
    async def channel_unlock(
        ...
        async def yes(
            ...
            if not data:
                ...
                already_unlocked_emb.set_footer(icon_url=ctx.author.display_avatar.url)
                try:
                    already_unlocked_emb.set_thumbnail(url=ctx.guild.icon.url)
                except AttributeError:
                    pass
                return await ui.response.edit(
                    embed=already_unlocked_emb,
                    view=ui,
                    allowed_mentions=self.bot.mentions,
                )
            channel_overwrites = channel.overwrites_for(snowflake)
            ...
            try:
                ...
                pass
            await ui.response.edit(
                embed=unlocked_emb, view=ui, allowed_mentions=self.bot.mentions
            )
        ...
        async def no(
            ...
            try:
                ...
                pass
            await ui.response.edit(
                embed=wont_unlock_emb, view=ui, allowed_mentions=self.bot.mentions
            )
    ...
    async def channel_list(self, ctx: BaseContext):
        """View members/roles that have been locked from certain channels"""
        fetched_locked_objects = await self.bot.db.fetch(
            "SELECT * FROM channel_lock WHERE guild_id = $1", ctx.guild.id
        )
        locked_objects = []
        serial_no = 0
        ...
        if not locked_objects:
            content = (
                f"**{ctx.author}** ─ there are no `roles` or `members` locked from any channels in **{ctx.guild}** <a:BlueGalHappy:915131837477687317>. "
