...
class Fun(commands.Cog):
        ...
        *,
        ...
        try:
            ...
            return await ctx.send(
                f"{ctx.author.mention} ─ please try again later. Either your internet is slow or mine is \U0001f91d"
            )

    @commands.command(name="nitro", brief="Gift Nitro")
    ...
    @commands.hybrid_command(name="pop", brief="Pop Buttons!", with_app_command=True)
    @app_commands.describe(
        flag="Add more buttons: --size <int> | Maximum buttons allowed is 25."
    )
    @commands.cooldown(3, 5, commands.BucketType.user)
    async def pop(
        self, ctx: BaseContext, *, flag: Optional[PopSize]
    ) -> Optional[discord.Message]:
    ...
    @click.command(name="start", brief="Start a game of click", with_app_command=True)
    @commands.cooldown(2, 10, commands.BucketType.user)
    @app_commands.describe(
        flag="Add more buttons: --size <int> | Maximum buttons allowed is 10."
    )
    async def click_start(
        self, ctx: BaseContext, *, flag: Optional[ClickSize]
    ) -> Optional[discord.Message]:
        ...
        if flag:
            if not flag.size:
                return await ctx.reply(
                    f"Please pass in a number for the number of buttons you want : `{ctx.clean_prefix}click start --size [int]`. Maximum buttons is 10."
                )
    ...
    async def urban(self, ctx: BaseContext, *, term: str) -> Optional[discord.Message]:
        ...
        try:
            async with self.bot.session.get(
                url="http://api.urbandictionary.com/v0/define", params={"term": term}
            ) as response:
                if response.status != 200:
                    return await ctx.reply(
                        f"**{ctx.author}** - An error occurred while connecting to Urban Dictionary:\n> **Status**: {response.status}\n? **Reason**: {response.reason}"
                    )
                ...
                if not data:
                    return await ctx.reply(
                        f"**{ctx.author}** - I was unable to find the definition for the term: `{term.strip()}`"
                    )

                embed_list: List[BaseEmbed] = []
                for results in data:
                    definition = results["definition"].replace("[", "").replace("]", "")
                    definition = (
                        definition
                        if len(definition) < 1024
                        else f"{definition[:1000]} . . ."
                    )
