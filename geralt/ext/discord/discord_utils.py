...
\x1b[0;1;37;40m > \x1b[0m \x1b[0;1;34mMessage Typing\x1b[0m \x1b[0;1;37;40m : \x1b[0m \x1b[0;1;31m{round(typing_ping, 1)} ms\x1b[0m\n```"""
    ...
    async def banner(self, ctx: BaseContext, *, user: Union[discord.Member, discord.User]=None) -> Optional[discord.Message]:
    ...
    async def avatar(self, ctx: BaseContext, *, user: Union[discord.User, discord.Member]=None) -> Optional[discord.Message]:
        ...
        if user == ctx.author:
            log_avatar = await self.bot.db.fetchval("SELECT avatar FROM user_settings WHERE user_id = $1", ctx.author.id)
            if not log_avatar:
    ...
    @commands.guild_only()
    async def userinfo(self, ctx: BaseContext, *, user: discord.Member = None) -> Optional[discord.Message]:
    ...
    @commands.command(
        ...
        aliases=["sp", "spot"])
    async def spotify(self, ctx: BaseContext, *, user: Union[discord.Member, discord.User]=None) -> Optional[discord.Message]:
