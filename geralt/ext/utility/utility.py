...
class Utility(commands.Cog):
    ...
    async def on_user_update(self, before: discord.User, after: discord.User):
        ...
        if after.bot:
            ...
            async with aiohttp.ClientSession() as session:
                bot_avatar_spam_webhook = discord.Webhook.partial(id=CONFIG.get(
                    "AVATAR_SPAM_ID"), token=CONFIG.get("AVATAR_SPAM_TOKEN"), session=session)
                sent_webhook_message = await bot_avatar_spam_webhook.send(file=discord.File(BytesIO(bot_avatar), filename=f"{after}'s_avatar.{imghdr.what(BytesIO(bot_avatar))}"), wait=True)
                await self.bot.db.execute("INSERT INTO avatar_history VALUES ($1, $2, $3, $4, $5)", after.id, str(sent_webhook_message.attachments[0]), discord.utils.utcnow(), imghdr.what(BytesIO(bot_avatar)), bot_avatar)
        ...
        if before.discriminator != after.discriminator:
            ...
            if log_discriminator:
                await self.bot.db.execute("INSERT INTO discriminator_history VALUES ($1, $2, $3)", after.id, after.discriminator, discord.utils.utcnow())
                return
    ...
    async def highlight_on_message_edit(self, before: discord.Message, after: discord.Message):
    ...
    async def todo(self, ctx: BaseContext, *, task_id: Optional[int]) -> Optional[discord.Message]:
    ...
    async def todo_add(self, ctx: BaseContext, *, task: Optional[str]) -> Optional[discord.Message]:
    ...
    async def todo_see(self, ctx: BaseContext, task_id: int) -> Optional[discord.Message]:
    ...
    async def todo_edit(self, ctx: BaseContext, task_id: int, *, edited: str) -> Optional[discord.Message]:
    ...
    async def todo_remove(self, ctx: BaseContext, *, task_id: int) -> Optional[discord.Message]:
    ...
    async def user_history(self, ctx: BaseContext, *, user: Optional[discord.User] = None):
    ...
    async def avatar_history(self, ctx: BaseContext, *, user: Optional[Union[discord.User, discord.Member]] = None):
        ...
        if not user.bot:
            log_avatar = await self.bot.db.fetchval("SELECT avatar FROM user_settings WHERE user_id = $1", user.id)
            if not log_avatar:
                return await ctx.reply(f"**{user}** - Hasn't `opted in` for avatar logging. Tell them to run `{ctx.clean_prefix}userlog avatar` <:TokoOkay:898611996163985410>")
        ...
        if len(avatar_history) == 1:
            for x in avatar_history:
                ...
                avatar_history_emb.set_footer(
                    text=f"Format: {x[3].capitalize()}")
    ...
    async def highlight_add(self, ctx: BaseContext, *, trigger: str = None):
        ...
        try:
            ...
            if highlight_data:
                ...
                self.bot.highlight = self.bot.generate_dict_cache(
                    highlight_data_list)
        except asyncpg.UniqueViolationError:
    ...
    async def highlight_remove(self, ctx: BaseContext, *, trigger: str = None):
        ...
        try:
            ...
            highlight_data_list: List = [
                (data["guild_id"],
                 data["user_id"],
                    data["trigger"]) for data in highlight_data]
            self.bot.highlight = self.bot.generate_dict_cache(
                highlight_data_list)
        except KeyError:
    ...
    async def highlight_block(self, ctx: BaseContext, *, object: Union[discord.Member, discord.Role]):
    ...
    async def highlight_unblock(self, ctx: BaseContext, *, object: Union[discord.Member, discord.Role]):
    ...
    async def highlight_blocked_list(self, ctx: BaseContext):
        ...
        if serial_no <= 3:
            ...
            while blacklisted:
                ...
                blacklisted_embs.set_thumbnail(
