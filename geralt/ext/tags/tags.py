...
class Tags(commands.Cog):
    ...
    async def tag_list(
        self, ctx: BaseContext, *, user: Optional[Union[discord.User, discord.Member]]
    ):
        ...
        if not tag_fetch:
            if user == ctx.author:
                return await ctx.reply(
                    f"**{user}** ─ You own no tags in `{ctx.guild}`. To create one, run `{ctx.clean_prefix}tag make` <:DuckSip:917006564265705482>"
                )
            if user == user:
                return await ctx.reply(
                    f"**{user}** ─ owns no tags in `{ctx.guild}`. To create one, run `{ctx.clean_prefix}tag make` <:DuckSip:917006564265705482>"
                )
        else:
            if serial_no <= 4:
                ...
                while tag_list:
                    tag_list_emb = BaseEmbed(
                        description="".join(tag_list[:3]), colour=self.bot.colour
                    )
                    tag_list_emb.set_author(name=f"{user}'s Tag List :")
                    tag_list_emb.set_thumbnail(url=user.display_avatar.url)
                    tag_list_emb.set_footer(
    ...
    async def tag_all(self, ctx: BaseContext):
        ...
        if not tag_fetch:
            await ctx.reply(
                f"**{user}** ─ There are no tags in `{ctx.guild}`. To create one, run `{ctx.clean_prefix}tag make` and you will get to know <:Okay:913796811737686086>"
            )
        else:
            if serial_no <= 4:
                tag_list_emb = BaseEmbed(
    ...
    async def tag_info(self, ctx: BaseContext, *, tag_name: commands.clean_content):
        ...
        if not tag_deets:
            return await ctx.reply(
                f"`{tag_name}` ─ is a tag either which is not present in this guild or not in my database."
            )
    ...
    async def tag_edit(self, ctx: BaseContext, tag_id: int, *, edited_content: str):
        if tag_id != await self.bot.db.fetchval(
            ...
            await ctx.reply(
                f"**{ctx.author}** ─ this is a tag which you either don't own or is not in the database."
            )
            ...
            class ContentButton(discord.ui.View):
                ...
                async def content_button(
                    ...
                    await interaction.response.send_message(
                        content=f"Content edited by {ctx.author.mention} :\n\n>>> ────\n{tag_deets[1]}\n────",
                        ephemeral=True,
                    )
                ...
                async def on_error(
                    ...
                    try:
                        await content_webhook.send(
                            content=f"```py\n{data}\n```\n|| Break Point ||"
                        )
                    except (discord.HTTPException, discord.Forbidden):
                        await content_webhook.send(
                            file=discord.File(
                                io.StringIO(data), filename="Traceback.py"
                            )
                        )
            ...
            await ctx.reply(
                f"Successfully edited Tag Name : `{tag_deets[0]}` (`{tag_id}`) ",
                view=ContentButton(),
            )
    ...
    async def tag_remove(self, ctx: BaseContext, name: str):
        ...
        async def yes(
            ...
            if id != await self.bot.db.fetchval(
                ...
                await ui.response.edit(
                    content=f'"**{tag_deets}**" ─ has been successfully deleted from your tag list <:NanoTick:925271358735257651>',
                    view=ui,
                )
                await ctx.add_nanotick()
    ...
    async def tag_transfer(self, ctx: BaseContext, tag_id: int, user: discord.Member):
        ...
        if user is None:
            return await ctx.send(
                f"Please mention a user to transfer the ownership of the tag."
            )
        ...
        async def yes(
            ...
            if id != await self.bot.db.fetchval(
                ...
                await ui.response.edit(
                    content=f'"**{tag_deets}**" (`{tag_id}`) ─ has been successfully transferred to {user.mention} <:NanoTick:925271358735257651>',
                    view=ui,
        ...
        async def no(
            ...
            await interaction.response.defer()
            await ui.response.edit(
                content=f'"**{tag_deets}**" (`{tag_id}`) ─ will not be transfered to {user.mention} <:NanoTick:925271358735257651>',
                view=ui,
                allowed_mentions=self.bot.mentions,
            )

            Confirmation.response = await ctx.reply(
                f'Are you sure you want to "transfer" tag ─ "**{tag_deets}**" (`{tag_id}`) to {user.mention} <:SIDGoesHmmMan:967421008137056276>',
                view=Confirmation(ctx, yes, no),
                allowed_mentions=self.bot.mentions,
            )
    ...
    async def edit_or_transfer_autocomplete(
        self, interaction: discord.Interaction, current: int
    ) -> List[app_commands.Choice[int]]:
        tag_deets = await self.bot.db.fetch(
            "SELECT (id) FROM tags WHERE guild_id = $1 AND author_id = $2 ORDER BY id ASC",
            interaction.guild_id,
            interaction.user.id,
        )
    ...
    async def user_tags_autocomplete(
        self, interaction: discord.Interaction, current: str
    ) -> List[app_commands.Choice[str]]:
        tag_deets = await self.bot.db.fetch(
    ...
    async def tag(
        self, ctx: BaseContext, *, tag_name: Optional[str]
    ...
    @tag.command(
        name="make", brief="Make a tag", aliases=["add"], with_app_command=True
    )
    @commands.cooldown(5, 5, commands.BucketType.user)
    ...
    @tag.command(name="raw", brief="Triggers Tag", aliases=["r"], with_app_command=True)
    ...
    async def _raw(
        self, ctx: BaseContext, *, tag_name: str
    ) -> Optional[discord.Message]:
        """See the tag without any markdown present."""
        await self.tag_raw(ctx, tag_name=tag_name)

    @tag.command(
        name="list", brief="Send user's tags list", aliases=["l"], with_app_command=True
    )
    @commands.cooldown(5, 5, commands.BucketType.user)
    ...
    @tag.command(name="all", brief="Send guild's tags list")
    @commands.cooldown(5, 5, commands.BucketType.user)
    async def _all(self, ctx: BaseContext):
        """Get a list of all tags present in this guild."""
        await self.tag_all(ctx)

    @tag.command(name="info", brief="Get info", with_app_command=True)
    @app_commands.rename(tag_name="name")
    @commands.cooldown(5, 5, commands.BucketType.user)
    @app_commands.autocomplete(tag_name=name_autocomplete)
    ...
    @tag.command(name="edit", brief="Edit a tag", with_app_command=True)
    ...
    async def _edit(
        self, ctx: BaseContext, tag_id: int, *, edited_content: str
    ) -> Optional[discord.Message]:
        """Edit a tags content which you own in this guild."""
        await self.tag_edit(ctx, tag_id=tag_id, edited_content=edited_content)

    @tag.command(
        name="delete", brief="Remove a Tag", aliases=["remove"], with_app_command=True
    )
    ...
    @tag.command(name="transfer", brief="Transfer Ownership", with_app_command=True)
    @app_commands.rename(tag_id="id")
    @commands.cooldown(5, 5, commands.BucketType.user)
    @app_commands.autocomplete(tag_id=edit_or_transfer_autocomplete)
    ...
    async def _transfer(
        self, ctx: BaseContext, tag_id: int, *, user: discord.Member
    ) -> Optional[discord.Message]:
        """Transfer a tag you own to another user in this guild."""
        await self.tag_transfer(ctx, tag_id=tag_id, user=user)

    @tag.command(name="import", with_app_command=False)
    @commands.cooldown(5, 5, commands.BucketType.user)
    async def tag_import(
        self, ctx: BaseContext, *, flag: Optional[TagFlags]
    ) -> Optional[discord.Message]:
        ...
        if flag:
            try:
                ...
                class ImportedContent(discord.ui.View):
                    ...
                    async def imported_tag_content(
                        self,
                        interaction: discord.Interaction,
                        button: discord.ui.Button,
                    ):
                        await interaction.response.send_message(
                            content=f"{tag_deets[1]}", ephemeral=True
                        )

                await ctx.reply(
                    f'Successfully transferred :\n\n> Tag Name : "`{tag_deets[0]}`"\n> Tag ID : `{id}`',
                    view=ImportedContent(),
                )
            except asyncpg.UniqueViolationError:
                return await ctx.send(
                    f'`{tag_deets[0]}` is a tag which is already present in "**{ctx.guild}**"'
                )
