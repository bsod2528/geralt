...
class Guild(commands.Cog):
    ...
    async def dont_archive_and_delete(self, ctx: BaseContext, ticket_id: int):
        ...
        ticket_kernel_list: List = [
            (data["guild_id"],
             data["ticket_id"],
                data["invoker_id"]) for data in ticket_kernel]
    ...
    async def emote_name_autocomplete(self, interaction: discord.Interaction, current: str) -> List[app_commands.Choice[str]]:
    ...
    async def prefix_add(self, ctx: BaseContext, *, prefix: Optional[str]) -> Optional[discord.Message]:
        """Add a custom prefix."""
    ...
    async def prefix_remove(self, ctx: BaseContext, *, prefix: Optional[str]) -> Optional[discord.Message]:
    ...
    async def ticket_setup(self, ctx: BaseContext, *, channel: discord.TextChannel=None) -> Optional[discord.Message]:
    ...
    async def ticket_close(self, ctx: BaseContext, ticket_id: Optional[int]) -> Optional[discord.Message]:
        """Close a ticket."""
        try:
            enabled_bool = self.bot.ticket_init[ctx.guild.id]
        except KeyError:
    ...
    async def verification_setup(self, ctx: BaseContext, channel: Optional[discord.TextChannel]) -> Optional[discord.Message]:
    ...
    @verification.command(
        ...
        aliases=["remove"])
    async def verification_dismantle(self, ctx: BaseContext) -> Optional[discord.Message]:
    ...
    async def guild_auditlog(self, ctx: BaseContext) -> Optional[discord.Message]:
        ...
        audit_log_emb = BaseEmbed(
            ...
            colour=self.bot.colour)
        message = await ctx.reply(f"Give me a second please <a:Load:1045668183434088479>", mention_author=False)
        await asyncio.sleep(1)
        view = MainAuditLog(self.bot, ctx, message.id)
        view.message = await message.edit(content=None, embed=audit_log_emb, view=view, allowed_mentions=self.bot.mentions)
    ...
    async def emote_delete(self, ctx: BaseContext, *, emote: discord.Emoji=None) -> Optional[discord.Message]:
        """Delete an emote."""
        if emote.guild.id != ctx.guild.id:
            return await ctx.reply(f"**{ctx.author} - {emote} is not present in **{ctx.guild}**!")
        ...
        async def yes(ui: discord.ui.View, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            try:
                ...
                return await ui.response.edit(content=f"**{ctx.author}** - Successfully deleted the emote <:WorryPray:941747031356104734>", view=ui)
            except discord.errors.HTTPException:
                return await ui.response.edit(f"**{ctx.author}** - I wasn't able to delete due to an error communicating with Discord <:Pain:911261018582306867>")

        async def no(ui: discord.ui.View, interaction: discord.Interaction, button: discord.ui.Button):
            for view in ui.children:
                view.disabled = True
            await interaction.response.defer()
            return await ui.response.edit(content=f"Seems like I'm not deleting {emote}. Won't do it **{ctx.author}** <:DuckThumbsUp:917007413259956254>")

        Confirmation.response = await ctx.reply(f"**{ctx.author}** - are you sure you want to delete {emote} - <:SIDGoesHmmMan:967421008137056276>", view=Confirmation(ctx, yes, no))
    ...
    async def emote_rename(self, ctx: BaseContext, emote: discord.Emoji=None, *, name: str) -> Optional[discord.Message]:
        """Rename an emote."""
        if emote.guild.id != ctx.guild.id:
            return await ctx.reply(f"**{ctx.author} - {emote} is not present in **{ctx.guild}**!")
        ...
        async def yes(ui: discord.ui.View, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            try:
                ...
                return await ui.response.edit(content=f"**{ctx.author}** - Successfully rename the emote to `{name}` <:WorryPray:941747031356104734>", view=ui)
            except discord.errors.HTTPException:
                return await ui.response.edit(f"**{ctx.author}** - I wasn't able to rename due to an error communicating with Discord <:Pain:911261018582306867>")

        async def no(ui: discord.ui.View, interaction: discord.Interaction, button: discord.ui.Button):
            for view in ui.children:
                view.disabled = True
            await interaction.response.defer()
            return await ui.response.edit(content=f"Seems like I'm not renaming {emote} to `{name}`. Won't do it **{ctx.author}** <:DuckThumbsUp:917007413259956254>")

        Confirmation.response = await ctx.reply(f"**{ctx.author}** - are you sure you want to rename {emote} to `{name}` - <:SIDGoesHmmMan:967421008137056276>", view=Confirmation(ctx, yes, no))
