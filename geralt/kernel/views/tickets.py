...
class SetupTicketPanel(discord.ui.Modal, title="Setup Your Panel"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            self.bot.ticket_init[self.ctx.guild.id] = [self.category_id.value.strip(),
                                                       self.channel.id,
...
class TicketSetup(discord.ui.View):
    ...
    @discord.ui.button(label="Panel Setup",
    ...
    async def send(self):
        ...
        ticket_setup_emb.add_field(
            name="Steps:", value="<:One:989876071052750868> Copy the `ID` of the Category you want ticket channels to be made.\n"
...
class CallTicket(discord.ui.View):
    ...
    async def call_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            try:
                user_channel: discord.TextChannel = await interaction.guild.create_text_channel(
                ...
                await self.bot.db.execute(f"INSERT INTO ticket_kernel (guild_id, invoker_id, channel_id) VALUES ($1, $2, $3)", interaction.guild.id, interaction.user.id, user_channel.id)
                ticket_id = await self.bot.db.fetchval("SELECT MAX(ticket_id) FROM ticket_kernel WHERE guild_id = $1", interaction.guild.id)
