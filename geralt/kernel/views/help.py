...
class HelpMenu(discord.ui.Select):
    ...
    def __init__(
        self, mapping: Dict[Cog, Command], help: BaseHelp, cog_list: List[Cog]
    ):
        options = [
            discord.SelectOption(
                label=cog.qualified_name, description=cog.description, emoji=cog.emote
            )
            for cog in cog_list
        ]
...
class HelpView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Home", style=discord.ButtonStyle.grey, emoji="\U0001f3d8", row=2
    )
    async def home(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for cog, commands in self.mapping.items():
            ...
            if filtered_cmds:
                ...
                help_emb.set_footer(
                    text=self.footer(), icon_url=self.ctx.author.display_avatar
                )
    ...
    async def delete(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            return await interaction.response.send_message(
                content="You cannot delete messages in ephemeral messages",
                ephemeral=True,
            )
...
class GroupAndCommandView(discord.ui.View):
    ...
    def footer(self):
        return (
            f"Run {self.help.context.clean_prefix}help [cog | command] for more info."
        )

    @discord.ui.button(label="Home", style=discord.ButtonStyle.grey, emoji="\U0001f3d8")
    async def home(self, interaction: discord.Interaction, button: discord.ui.Button):
        help_emb = BaseEmbed(
        ...
        for cog, commands in self.mapping.items():
            ...
            if filtered_cmds:
                ...
                help_emb.set_footer(
                    text=self.footer(), icon_url=self.help.context.author.display_avatar
                )
