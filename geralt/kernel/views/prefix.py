    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        try:
            ...
            prefix_emb = BaseEmbed(
                description=f"> <:GeraltRightArrow:904740634982760459> " + "\n> <:GeraltRightArrow:904740634982760459> ".join(await self.bot.get_prefix(self.ctx.message)),
                colour=self.bot.colour)
            prefix_emb.set_footer(
                text=f"Run {self.ctx.clean_prefix}help prefix.")
            if self.ctx.guild.icon.url:
                prefix_emb.set_author(name=f"{len(await self.bot.get_prefix(self.ctx.message))} Prefixes ─ {self.ctx.guild}", icon_url=self.ctx.guild.icon.url)
            else:
                prefix_emb.set_author(name=self.ctx.guild)
            prefix_view = Prefix(self.bot, self.ctx)
            prefix_view.message = await interaction.message.edit(embed=prefix_emb, view=prefix_view)
...
class RemovePrefix(discord.ui.Modal, title="Remove a Prefix"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        try:
            ...
            for guild_id, prefixes in data:
                self.bot.prefixes[guild_id] = set(prefixes)
            await interaction.response.send_message(content=f"`{self.prefix.value.strip()}` - Has been removed from my prefix list for **{interaction.guild.name}** <:SarahLaugh:907109900952420373>", ephemeral=True)
