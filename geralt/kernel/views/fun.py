...
class ClickGame(discord.ui.View):
    ...
    @discord.ui.button(
        label="Scores", style=discord.ButtonStyle.grey, emoji="\U00002728", row=2
    )
    async def on_click_guild_leaderboard(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        try:
            guild_score_query = await self.bot.db.fetchval(
                "SELECT (clicks) FROM click_guild WHERE guild_id = $1 AND player_id = $2",
                interaction.guild_id,
                self.ctx.author.id,
            )
            await interaction.response.defer()
            if not guild_score_query:
                await interaction.followup.send(
                    content=f"{self.ctx.author.mention} has yet to click on the button in **{interaction.guild.name}** <:TokoOkay:898611996163985410>",
                    ephemeral=True,
                )
            else:
                await interaction.followup.send(
                    content=f"{self.ctx.author.mention} has clicked a total of `{guild_score_query}` times in **{interaction.guild.name}** <:TokoOkay:898611996163985410>",
                    ephemeral=True,
                )
        except Exception as exception:
            try:
    ...
    async def send(self, ctx):
        self.message = await ctx.reply(
            content="Click as fast as you can and earn points <a:CrazyRolex:966181282914660402>",
            view=self,
        )
        return self.message
    ...
    async def on_timeout(self) -> None:
        ...
        return await self.message.edit(
            content=f"**{self.ctx.author}** - you've been timed out due to inactivity. To start clicking again, rerun `{self.ctx.clean_prefix}{self.ctx.command}` and be the number one on the leaderboard <a:Comfort:918844984621428787>",
            view=self,
        )
...
class ClickLeaderboard(discord.ui.View):
    ...
    async def click_global_leaderboard(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        try:
            await interaction.response.defer()
    ...
    async def send(self):
        guild_score_query = await self.bot.db.fetch(
            "SELECT player_id, clicks FROM click_guild WHERE guild_id = $1 ORDER BY clicks DESC LIMIT 10",
            self.ctx.guild.id,
        )
        serial_no = 1
        leaderboard = []
        ...
        if not guild_score_query:
            return await self.ctx.reply(
                f"No one from **{self.ctx.guild}** has played `{self.ctx.clean_prefix}click` game <a:Noo:915422306896072744>. Feel honoured and be the first one !"
            )
        else:
            while leaderboard:
                ...
                leaderboard_emb.add_field(
                    name="Top 10 Scores", value="".join(leaderboard[:10])
                )
        ...
        self.message = await self.ctx.reply(
            embed=leaderboard_emb, view=self, mention_author=False
        )
        return self.message
