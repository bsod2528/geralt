...
class UserHistory(discord.ui.View):
    ...
    async def see_username_history(
        ...
        data = [
            ...
            for deets in usernames
        ]
        if not usernames:
            return await interaction.response.send_message(
                content=f"{self.user.mention} ─ Has no records for changes in usernames logged in <:EyesGoBrr:965662700627710032>. To start logging, run `{self.ctx.clean_prefix}log username`",
                ephemeral=True,
            )
        username_emb = BaseEmbed(description="\n".join(data), colour=self.bot.colour)
    ...
    async def delete_userhistory_message(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        if interaction.user.id != self.ctx.author.id:
            return await interaction.response.send_message(
                content="Nope", ephemeral=True
            )
...
class SelectUserLogEvents(discord.ui.View):
    ...
    async def opt_avatar(
        ...
        data = await self.bot.db.fetchval(
            "SELECT avatar FROM user_settings WHERE user_id = $1", interaction.user.id
        )
        if data:
            await self.bot.db.execute(query, interaction.user.id, False)
            return await interaction.response.send_message(
                content=f"**{interaction.user}** ─ Successfully `opted out` from avatar logging <:TokoOkay:898611996163985410>",
                ephemeral=True,
            )

        await self.bot.db.execute(query, interaction.user.id, True)
        await interaction.response.send_message(
            content=f"**{interaction.user}** ─ Successfully `opted in` from avatar logging <:DuckThumbsUp:917007413259956254>",
            ephemeral=True,
        )

    @discord.ui.button(label="Username", style=discord.ButtonStyle.blurple)
    async def opt_username(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        query = (
        ...
        data = await self.bot.db.fetchval(
            "SELECT username FROM user_settings WHERE user_id = $1", interaction.user.id
        )
        if data:
            await self.bot.db.execute(query, interaction.user.id, False)
            return await interaction.response.send_message(
                content=f"**{interaction.user}** ─ Successfully `opted out` from username logging <:TokoOkay:898611996163985410>",
                ephemeral=True,
            )

        await self.bot.db.execute(query, interaction.user.id, True)
        await interaction.response.send_message(
            content=f"**{interaction.user}** ─ Successfully `opted in` from username logging <:DuckThumbsUp:917007413259956254>",
            ephemeral=True,
        )
