...
class SetupVerificationModal(discord.ui.Modal, title="Setup Verification"):
    ...
    question = discord.ui.TextInput(
        label="Question", required=True, placeholder='Your "Question" goes here.'
    )

    answer = discord.ui.TextInput(
        label="Answer", required=True, placeholder='Your "Answer" goes here.'
    )

    role_id = discord.ui.TextInput(
        label="ID of the Role", required=True, placeholder="ID of the role to give."
    )
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        verify_emb.add_field(name="Question:", value=f"> {self.question.value.strip()}")
        verify_emb.add_field(
            name="Answer:", value=f"> {self.answer.value.strip()}", inline=False
        )
        try:
        ...
        verification_sent_message = await self.channel.send(
            embed=verify_emb, view=VerificationCall(self.bot, self.ctx)
        )
        ...
        await self.bot.db.execute(
            ...
            str(self.channel.id),
            str(verification_sent_message.id),
        )
        self.bot.verification[interaction.guild.id] = [
            self.question.value.strip(),
            self.answer.value.strip(),
            self.role_id.value.strip(),
            str(self.channel.id),
...
class AnswerVerification(discord.ui.Modal, title="Answer the verification"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        try:
            ...
            actual_answer = await self.bot.db.fetchval(
                "SELECT answer FROM verification WHERE guild_id = $1",
                interaction.guild.id,
            )
        if actual_answer != self.answer.value.strip():
            return await interaction.response.send_message(
                content=f"`{self.answer.value.strip()}` is the incorrect answer. Please check properly and retry.",
                ephemeral=True,
            )
        if actual_answer == self.answer.value.strip():
            try:
                ...
                role = await self.bot.db.fetchval(
                    "SELECT role_id FROM verification WHERE guild_id = $1",
                    interaction.guild.id,
                )
            try:
                await interaction.user.add_roles(
                    discord.Object(role), reason=f"Got verified by {self.bot.user}"
                )
                await interaction.response.send_message(
                    content=f"**{interaction.user}** ─ you have been verified successfuly and got the <@&{role}>",
                    ephemeral=True,
                )
            except discord.errors.Forbidden:
                return await interaction.response.send_message(
                    content=f"Please contact the `{interaction.guild}'s` admin and tell them that <@&{role}> is higher than my role in the role order <a:Grimacing:914905757588283422>",
                    ephemeral=True,
                )
            except discord.HTTPException:
                return await interaction.response.send_message(
                    content=f"**{interaction.user}** ─ an error has occured. Please try again in a while",
                    ephemeral=True,
                )
...
class SetupVerification(discord.ui.View):
    ...
    async def setup_verification(
        ...
        await interaction.response.send_modal(
            SetupVerificationModal(self.bot, self.ctx, self.channel)
        )
...
class VerificationCall(discord.ui.View):
    ...
    async def call_for_verification(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
