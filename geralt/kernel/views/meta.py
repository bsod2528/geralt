...
class PFP(discord.ui.View):
    ...
    @discord.ui.button(label="Log",
                       style=discord.ButtonStyle.green,
                       emoji="<a:PandaNote:961260552435413052>")
    async def save(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            user = self.user
            all_pfps = await self.bot.db.fetch("SELECT * FROM avatar_history WHERE user_id = $1", user.id)
            current_pfp = await self.ctx.author.display_avatar.read()
            for pfp in all_pfps:
                if current_pfp in pfp[4]:
                    button.disabled = True
                    await interaction.message.edit(view=self)
                    return await interaction.response.send_message(content=f"**{user}** - This PFP has already been logged <a:DuckPopcorn:917013065650806854>", ephemeral=True)
            ...
            async with aiohttp.ClientSession() as session:
                avatar_spam_webhook = discord.Webhook.partial(
                    ...
                    session=session)
                sent_webhook_message = await avatar_spam_webhook.send(file=discord.File(BytesIO(avatar), filename=f"{user}'s_avatar.{imghdr.what(BytesIO(avatar))}"), wait=True)
                await self.bot.db.execute("INSERT INTO avatar_history VALUES ($1, $2, $3, $4, $5)", user.id, str(sent_webhook_message.attachments[0]), discord.utils.utcnow(), imghdr.what(BytesIO(avatar)), avatar)
                await avatar_spam_webhook.session.close()
                button.disabled = True
                await interaction.message.edit(view=self)
                return await interaction.response.send_message(content=f"**{user}** - Your PFP has been logged <a:Click:973748305416835102>", ephemeral=True)
...
class FeedbackModal(discord.ui.Modal, title="Feedback Form"):
    ...
    async def on_submit(self, interaction: discord.Interaction, /) -> None:
        fb_info = f"- Sent By       :   {self.ctx.author} / {self.ctx.author.id}\n" \
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                await session.close()
            return await interaction.response.send_message(content=f"**{self.ctx.author}** - Your feedback form has been sucessfully sent <a:Nod:1064213975031627897>", ephemeral=True)
            ...
            return await interaction.response.send_message(
                ...
                "Click on the link below to gain access to the server!", ephemeral=False, view=support_server_link)
...
class BugModal(discord.ui.Modal, title="Bug Form"):
    ...
    async def on_submit(self, interaction: discord.Interaction, /) -> None:
        bug_emb = f"- Sent By       :   {self.ctx.author} / {self.ctx.author.id}\n" \
                  f"- @ Guild       :   {self.ctx.guild} / {self.ctx.guild.id}\n" \
                  f"- @ Channel     :   {self.ctx.channel} / {self.ctx.channel.id}"
        bug_emb = BaseEmbed(
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                await feedback_webhook.send("||  Break Point  ||")
                await session.close()
            return await interaction.response.send_message(content=f"**{self.ctx.author}** - Your bug form has been sucessfully sent <a:Nod:1064213975031627897>", ephemeral=True)
        except Exception as exception:
            support_server_link: discord.ui.View = discord.ui.View()
            support_server_link.add_item(
                discord.ui.Button(
