...
class BaseHelp(commands.HelpCommand):
    ...
    def extra_footer(self,
                     group_or_command: Union[commands.Group,
    ...
    def get_signature(
        ...
        if group_or_command.aliases:
            alias = " | ".join(
                f"`{alias}`" for alias in sorted(
                    group_or_command.aliases))
        else:
            alias = "`Nil`"
    ...
    async def send_bot_help(self, mapping) -> Optional[discord.Message]:
        ...
        main_help_view.message: discord.Message = await self.context.reply(embed=help_emb, mention_author=False, view=main_help_view)
