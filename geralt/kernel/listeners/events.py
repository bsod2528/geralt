...
class Events(commands.Cog):
    ...
    async def send_emote_url(self, message: discord.Message):
        try:
            try:
                convert_url_to_webhook = self.bot.settings[message.guild.id][
                    "convert_url_to_webhook"
                ]
            except KeyError:
                convert_url_to_webhook = await self.bot.db.fetchval(
        ...
        try:
            ...
            if convert_url_to_webhook == True:
                ...
                emote_prefix = {True: "a", False: ""}[emote.group("fmt") == "gif"]
                ...
                if message.reference:
                    ...
                    referenced_message_emb.set_author(
                        name=f"Replying to {message.reference.resolved.author}",
    ...
    async def edit_to_url(self, before: discord.Message, after: discord.Message):
        try:
            try:
                convert_url_to_webhook = self.bot.settings[after.guild.id][
                    "convert_url_to_webhook"
                ]
            except KeyError:
                convert_url_to_webhook = await self.bot.db.fetchval(
        ...
        try:
            ...
            if convert_url_to_webhook == True:
                ...
                if after.reference:
                    ...
                    referenced_message_emb.set_author(
                        name=f"Replying to {after.reference.resolved.author}",
                        icon_url=after.reference.resolved.author.display_avatar,
                    )
                    return await webhook.send(
                        content=f"<{emote_prefix}:_:{emote_id}>",
                        embed=referenced_message_emb,
    ...
    async def on_command_completion(self, ctx: BaseContext):
    ...
    async def on_guild_join(self, guild: discord.Guild):
        ...
        join_emb = BaseEmbed(
            title=f":scroll: I Joined {guild.name}", colour=self.bot.colour
        )
        join_emb.add_field(
