...
class GeraltHelp(commands.HelpCommand):
    ...
    async def send_cog_help(self, cog: commands.Cog) -> Optional[discord.Message]:
        ...
        for commands in filtered_commands:
            ...
            cog_emb.set_footer(text=self.main_footer(),
                               icon_url=self.context.author.display_avatar)
    ...
    async def send_command_help(self, command: commands.Command) -> Optional[discord.Message]:
        ...
        command_emb.set_footer(
            text=self.extra_footer(command),
            icon_url=self.context.author.display_avatar)
        ...
        command_view: discord.ui.View = GroupAndCommandView(self, HelpCommand.get_bot_mapping(self))
        command_view.message = await self.context.reply(embed=command_emb, mention_author=False, view=command_view)
    ...
    async def send_group_help(self, group: commands.Group) -> Optional[discord.Message]:
        try:
            ...
            return
        group_commands: List = [
            f"> <:Join:932976724235395072> `{command.name}{' '*(10 - len(command.name))}` \u200b ─ \u200b {command.short_doc[:40]}" for command in group.commands]
        ...
        if len(group_commands) > 3:
            ...
            return await Paginator(self.context.bot, self.context, embeds=embed_list).send(self.context)
        ...
        group_view: discord.ui.View = GroupAndCommandView(self, HelpCommand.get_bot_mapping(self))
        group_view.message = await self.context.reply(embed=group_emb, mention_author=False, view=group_view)
